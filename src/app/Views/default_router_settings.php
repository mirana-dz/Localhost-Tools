...
?>
    ...
    <div id="header-search">
        <div class="input-group">
            ...
            <input type="text" id="search-box-brand" class="search-input"
                   placeholder="Type any word to search brands"
                   autocomplete="off">
        </div>
        <hr>
    </div>
    ...
    <script>
        $(document).ready(function () {
            $("#index-results").load("?route=default_router_settings&action=load");
        });
        ...
        function doAjax(page, search) {
            $.ajax({
                url: '?route=default_router_settings&action=load' + page + '&search=' + search,
                type: 'get',
                beforeSend: function () {
